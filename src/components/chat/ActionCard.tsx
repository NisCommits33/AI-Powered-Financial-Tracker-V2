...
type Status = "pending" | "done" | "cancelled" | "error";
...
export function ActionCard({ action }: { action: ChatAction }) {
  ...
  if (action.type === "create_transaction") {
    const { amount, description, category, date, account_id, account_name } = action.args;
    return (
      <ActionShell
  ...
  if (action.type === "set_accent_color") {
  ...
  if (action.type === "set_dashboard_charts") {
    ...
    return (
      <ActionShell
        ...
        onCancel={cancel}
      >
        <p>Show: {labels.length > 0 ? labels.join(", ") : "none"}</p>
      </ActionShell>
    );
  }

  if (action.type === "update_account_balance") {
  ...
  if (action.type === "set_button_radius") {
    ...
    return (
      <ActionShell
        ...
        onCancel={cancel}
      >
        <p className="font-semibold text-foreground">{def?.label || radius}</p>
      </ActionShell>
    );
  }
